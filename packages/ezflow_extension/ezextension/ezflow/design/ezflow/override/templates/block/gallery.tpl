...
<div id="{$block.id}" class="dailyPhoto">
    ...
    {foreach $block.valid_nodes as $index => $item}
        {if eq( $index, 0 )}
        <div style="z-index: 1; opacity: 0.999999;" id="{$block.id}_i{$index}"><a href="{$item.url_alias|ezurl(no)}"><img alt="" src="{$item.data_map.image.content[galleryblock].full_path|ezroot(no)}" /></a></div>
        {else}
        <div style="z-index: -{$index}; opacity: 0;" id="{$block.id}_i{$index}" class=""><a href="{$item.url_alias|ezurl(no)}"><img alt="" src="{$item.data_map.image.content[galleryblock].full_path|ezroot(no)}" /></a></div>
        {/if}
    {/foreach}
    </div>
    ...
    {foreach $block.valid_nodes as $index => $item}
        ...
        <div style="z-index: 1; opacity: 0.999999;" id="{$block.id}_ic{$index}">
            <div><a href="{$item.url_alias|ezurl(no)}">{$item.data_map.caption.content.output.output_text}</a></div>
        </div>
        {else}
